...
interface NewsDataSource {
    ...
    suspend fun list(sort: SortingMode): PaginatedData<Article>

    suspend fun loadMore(lastPage: Int, sort: SortingMode): PaginatedData<Article>

    class Impl(private val service: NewsService) : NewsDataSource {
        override suspend fun list(sort: SortingMode) = loadPage(1, sort)

        override suspend fun loadMore(lastPage: Int, sort: SortingMode) =
            try {
        ...
        private suspend fun loadPage(page: Int, sort: SortingMode): PaginatedData<Article> {
